...
function App() {
  ...
  // Generate a random agent ID on load (or you can set a fixed one)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const socket = io(API_BASE, {
      ...
      auth: { token: localStorage.getItem('agentToken') || '' },
    });
    socketRef.current = socket;

    socket.on('connect', () => {
      socket.emit('join-room', 'agent-dashboard');
          ...
          tickets
            ...
            .map((ticket) => (
              ...
              >
                ...
                <div style={styles.ticketCompany}>Company: {ticket.companyId}</div>
