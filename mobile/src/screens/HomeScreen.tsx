...
export default function HomeScreen() {
  ...
  const isCompanyLeader = user?.role === 'boss' || user?.role === 'manager';
  const toolSections = [
    {
      title: 'Work & Activity',
      actions: [
        { icon: 'history', color: '#7C4DFF', label: 'History', screen: 'History' },
        { icon: 'assignment', color: '#4CAF50', label: 'Tasks', screen: 'Tasks' },
    ...
    ...(isCompanyLeader ? [{
      ...
      actions: [
        ...
        { icon: 'people', color: '#00D4FF', label: 'Team & Roles', screen: 'Team' },
        { icon: 'qr-code', color: '#9C27B0', label: 'Kiosk', screen: 'Kiosk' },
        { icon: 'settings', color: '#00BCD4', label: 'Company Settings', screen: 'CompanySettings' },
      ],
    }] : []),
    {
      title: 'Account & Help',
      actions: [
        { icon: 'person', color: '#00D4FF', label: 'My Settings', screen: 'Profile' },
    ...
    <View style={styles.wrapper}>
      ...
      >
        ...
        {toolSections.map(section => (
          <View key={section.title} style={styles.toolSection}>
            <Text style={styles.toolSectionTitle}>{section.title}</Text>
            <View style={styles.moreActions}>
              {section.actions.map(action => (
                <TouchableOpacity key={action.screen} style={styles.moreActionBtn} onPress={() => navigation.navigate(action.screen)}>
...
const styles = StyleSheet.create({
  ...
  toolSectionTitle: { color: '#8A94A6', fontSize: 12, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase', paddingHorizontal: 20, marginBottom: 10 },
