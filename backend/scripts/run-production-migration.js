...
-- Fixed: final_pay computes directly (no reference to generated 'pay')
...
-- 3. INVOICING TABLES (FIXED: no nested generated columns)
CREATE TABLE IF NOT EXISTS invoices (
  ...
  balance DECIMAL(10,2) GENERATED ALWAYS AS ((subtotal + (subtotal * tax_rate / 100)) - paid_amount) STORED,
  notes TEXT,
  client_notes TEXT,
  sent_at TIMESTAMP,
  viewed_at TIMESTAMP,
...
SELECT 'All done' as final_step;
`;

async function run() {
  console.log('🔄 Starting production migration (dropping & recreating tables)...');
  const connected = await testConnection();
  if (!connected) {
    console.log('❌ Cannot proceed – check your DATABASE_URL.');
    process.exit(1);
  }
