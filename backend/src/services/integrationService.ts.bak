...
export async function syncStripeEventToQuickBooks(stripeEvent: any): Promise<void> {
  ...
  try {
    ...
    if (aiDecision.action === 'createSalesReceipt') {
      const { createSalesReceipt } = await import('./quickbooksService');
      await createSalesReceipt(
        companyId,
        aiDecision.customerRef || '1',
        eventData.amount / 100,
        aiDecision.description || 'Payment received',
        txnDate
      );
    } else if (aiDecision.action === 'createInvoicePayment') {
      const { createInvoicePayment } = await import('./quickbooksService');
      await createInvoicePayment(
        companyId,
        aiDecision.invoiceId,
    ...
    await pool.query(
      `INSERT INTO sync_logs (company_id, provider, event_type, status, request_data, ai_decision, error_message)
       VALUES ($1,'stripe->quickbooks',$2,'failed',$3,$4,$5)`,
      [companyId, stripeEvent.type, JSON.stringify(stripeEvent), JSON.stringify(aiDecision), error.message]
    );
    throw error;
  }
}
